    ...
    Alpine.directive(
        ...
        (
            ...
            { expression, modifiers },
            {
                ...
                evaluateLater: (expression: string) => (value: unknown) => void;
                effect: (callback: () => void) => void;
            },
        ) => {
            let config: AutoAnimateConfig = {};
            const durationModifier: string =
                modifiers.filter((modifier: string) =>
            ...
            const controller: AnimationController = autoAnimate(
                <HTMLElement>el,
                { ...autoAnimateConfig, ...config },
            );
            if (expression) {
                const isEnabled: (callback: (value: boolean) => void) => void =
